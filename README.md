...
框架不是一次性设计，而是在五届全国级竞赛中实飞迭代沉淀而来——从研读学长的 27 届国赛代码起步、完成比赛后自主设计五层架构，其后每届都在实战中演进：

| 阶段 | 场景 | 代码形态 | 关键演进 | 规模（.py / 行 / 自定义接口） |
|------|------|---------|---------|:---:|
| 第 27 届 CRAIC 国赛 | 无人机竞赛任务 | 脚本式：`demo.py` + 巨石库 `uav_library.py` | 首次完整实飞，暴露脚本堆叠难维护问题 | 10 / ~3800 / 0 |
...
**脚本式时代**（27 届 CRAIC / 2025 H）——所有逻辑平铺在 `scripts/` 根目录，模型权重与代码混放：
...
27 届的 `uav_library.py` 是理解本框架演进的最佳样本——一个 `uav_lib` 类同时承担六种职责，重构后各自归入五层：

| 当年的职责（27 届 `uav_lib` 类） | 如今的归属（2026 D 完善版） |
|------|------|
...
**进化的本质**：不是推倒重写，而是职责归位——2025 H 仓库至今保留着 27 届代码备份（`legacy/`），见证逐届继承；2024 D 起每届只重写"必改层"，执行/定位/安全/记录持续复用至今。
...
**框架的由来**：我的无人机竞赛之路，从逐行研读学长留下的第 27 届 CRAIC 国赛代码开始——学透并用它完成比赛后，我针对脚本式架构在维护与复用上的痛点，结合社区工程实践自主设计了这套五层解耦框架，并在其后每一届赛事中实飞验证、逐题优化至今。

感谢学长的代码启蒙、每一次比赛并肩调试到深夜的队友，以及开源社区：[pymavlink](https://github.com/ArduPilot/pymavlink)（MAVLink 协议通信）、[FAST-LIO](https://github.com/hku-mars/FAST_LIO)（激光惯性里程计，定位层输入源）、ROS / catkin 社区。
