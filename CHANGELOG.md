...
框架并非一次性设计，以下为历次实战迭代记录。每届任务均完成全流程实飞验证。
...
- **第一次整体重构**：脚本式代码拆分为 common / decision / execution / localization / perception / planning / safety / logging 分层结构
- 公共层接口集中管理（`uav_interfaces.py`）、安全旁路与记录层建立

### 2025 · 野生动物巡查系统 —— 脚本式框架延续
- 新增地面站通信（`gs_comm`）与网格全覆盖路径规划（`path_planner`），为后续五层化打下模块基础

### 第 28 届 CRAIC 国赛 —— 框架雏形
- 脚本式实现（`demo.py` + 功能库），首次完成完整实飞，暴露脚本堆叠在维护与复用上的问题，催生分层设计思想
